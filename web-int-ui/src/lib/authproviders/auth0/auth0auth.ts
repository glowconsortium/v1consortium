...
class Auth0Store {
  ...
  // Keep the last appState returned by Auth0 after redirect
  private lastAppState: any | null = null;
  ...
  async init(config: Auth0Config): Promise<void> {
    try {
      ...
      // Handle redirect if code/state present (per 01-Login sample)
      const hasAuthParams =
        window.location.search.includes('code=') && window.location.search.includes('state=');

      if (hasAuthParams) {
        console.log('Auth0 redirect detected, handling callback...');
        await this.handleRedirectCallback();
      } else {
        console.log('No redirect params found, skipping redirect handling');
      }
  ...
  private async handleRedirectCallback(): Promise<void> {
    ...
    if (urlParams.has('code') && urlParams.has('state')) {
      ...
      try {
        ...
        // Save appState for the callback page to use
        this.lastAppState = result?.appState ?? null;
        console.log('Auth0 callback handled successfully. appState:', this.lastAppState);
  ...
  // Expose appState captured from the callback
  ...
  hasRole(role: string): boolean {
    ...
    const roles = (u && (u['https://example.com/roles'] || u.roles)) || [];
  ...
  hasPermission(permission: string): boolean {
    if (!this.user) return false;
    const u: any = this.user as any;
    const permissions = (u && (u['https://example.com/permissions'] || u.permissions)) || [];
